...
-- ============================================
-- Ye file MySQL mein run karein - sab APIs kaam karne lag jayengi

USE truck_management;

-- Step 1: Companies table create karo
...
-- Step 2: Users table mein company_id add karo
...
SET @sql = IF(@exists = 0, 
              ...
              'SELECT "users.company_id already exists" AS msg');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
UPDATE users SET company_id = 1 WHERE company_id IS NULL OR company_id = 0;

-- Step 3: Drivers table mein company_id add karo
SET @exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
               WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'drivers' AND COLUMN_NAME = 'company_id');
SET @sql = IF(@exists = 0, 
...
-- Step 4: Customers table mein company_id add karo
SET @exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
...
-- Step 5: Trucks table mein company_id add karo
SET @exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
               WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'trucks' AND COLUMN_NAME = 'company_id');
SET @sql = IF(@exists = 0, 
...
-- Step 6: Tickets table mein company_id add karo (YEH MAIN ERROR HAI)
SET @exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
               WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'tickets' AND COLUMN_NAME = 'company_id');
SET @sql = IF(@exists = 0, 
...
-- Step 7: Master data tables create karo
...
-- Success message
SELECT '✅ SUCCESS! Sab tables fix ho gaye!' AS Status;
